package ClassicInterviewCoding.InterviewCoding02;
...
public class Code07_KthMinPair {
	...
	public static int[] kthMinPair1(int[] arr, int k) {//暴力方法，生成好数对的数组然后排序
		int N = arr.length;
		if (k > N * N) {
			return null;
		}
	...
	// O(N)的复杂度，你肯定蒙了
	public static int[] kthMinPair3(int[] arr, int k) {//不需要对arr事先排序
		int N = arr.length;
		if (k > N * N) {
			return null;
		}
