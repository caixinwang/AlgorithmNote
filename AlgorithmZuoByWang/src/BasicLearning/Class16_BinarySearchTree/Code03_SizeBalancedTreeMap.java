...
public class Code03_SizeBalancedTreeMap {
	...
	public static class SizeBalancedTreeMap<K extends Comparable<K>, V> {
		...
		/**
		 *
		 * @param key 先在有序表里面找有没有对应的key，如果有就把对应的Node返回，如果没有--就说明滑到空了，就返回最近的非空结点
		 * @return 如果root为空，最终就返回空
		 */
		...
		private SBTNode<K, V> add(SBTNode<K, V> cur, K key, V value) {
			if (cur == null) {
				return new SBTNode<K, V>(key, value);//返回给上游结点用
			} else {
				cur.size++;
				if (key.compareTo(cur.key) < 0) {
		...
		//lastNode == null说明是一棵空树
		//key.compareTo(lastNode.key) == 0 用来判断在findLastIndex过程中，是找到了对应的key break掉的，还是滑到空的上一个结点。
		public void put(K key, V value) {
			if (key == null) {
				throw new RuntimeException("invalid parameter.");
			}
			SBTNode<K, V> lastNode = findLastIndex(key);
			if (lastNode != null && key.compareTo(lastNode.key) == 0) {//判断是改值还是新增
