...
// 本节在尝试上，最难的
// 搞出桶来，组合公式
public class Code0030_SequenceKDifferentKinds {
	...
	public static int nums2(String s, int k) {
		char[] str = s.toCharArray();
	...
	private static int g(int[] count, int index, int rest) {
		if (index==count.length){
			return rest==0?1:0;
		}
		int p1=0,p2=0;
		if (count[index]>0){
			p1=math(count[index])*g(count, index+1, rest-1);
		}
		p2=g(count, index+1, rest);
		return p1+p2;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < times; i++) {
			String s = su.generateRandom_a_z_String(su.ran(1,10));
			int k = su.ran(1,s.length());
			int ans1=nums(s,k);
			int ans2=nums2(s,k);
			if (ans1!=ans2){
