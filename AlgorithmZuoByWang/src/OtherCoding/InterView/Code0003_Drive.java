...
public class Code0003_Drive {
	...
	public static int maxMoney2(int[][] matrix) {
		int n=matrix.length;
		int[][] dp=new int[n+1][n/2+1];//dp[i][j]前i个司机自由发派，恰好发配了j个司机到a区域
		for(int[] a:dp) Arrays.fill(a,-1);
		dp[0][0]=0;
		for(int i=1;i<=n;i++){
			for (int j=0;j<=n/2;j++){
				//一定要加判断，不能从不合法的位置转移来
				if (dp[i-1][j]!=-1) dp[i][j]=dp[i-1][j]+matrix[i-1][1];//发到b去
				if (j-1>=0&&dp[i-1][j-1]!=-1) dp[i][j]=Math.max(dp[i][j],dp[i-1][j-1]+matrix[i-1][0]);//发a去
			}
		}
		return dp[n][n/2];
	}
	...
	public static void main(String[] args) {
		...
		for (int i=0;i<1000000;i++){
			int[][] matrix = au.generateRandomMatrix(au.ran(1,4)*2,2,1,100);
			int res1=maxMoney(matrix);
			int res2=maxMoney2(matrix);
			if (res1!=res2){
				System.out.println(res1);
				System.out.println(res2);
				au.printMatrix(matrix);
			}
		}
	}
