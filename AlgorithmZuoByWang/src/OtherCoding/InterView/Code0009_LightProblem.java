 ...
 * 
 * */
public class Code0009_LightProblem {
	...
	/**
	 * 这个肯定是最优解，O(N)，一个位置按不按其实只取决于前面的灯，因为如果是从左往右的尝试模型，你前面的灯只能由你决定，
	 * 所以在你这个位置i走了之后，要保证i-1一定是亮的。所以这就是一个O(N)的过程模拟题。从1~N-1的位置选择都是由不得我们的。
	 * 所以最后检查一下N-1位置亮不亮即可。
	 * 0位置可以任意选择，因为0位置前面没有任何东西。
	 */
	public static int noLoopMinStep3(int[] arr) {
		if (arr == null || arr.length == 0) return 0;
		int[] cg=new int[arr.length];
		System.arraycopy(arr,0,cg,0,arr.length);
		reverse(cg,0);
		int p1=1;
		...
		if (cg[cg.length-1]==0) p1=Integer.MAX_VALUE;
		int p2=0;
		...
		if (arr[arr.length-1]==0) p2=Integer.MAX_VALUE;
		return Math.min(p1,p2);
	}
