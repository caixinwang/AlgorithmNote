...
public class Code0034_AllSame {
	...
	public static int allSame3(int[] arr) {
		int ans=arr.length;
		for (int i = 0; i < arr.length; i++) {
			ans = Math.min(ans, f(arr,i,i,true));
		}
		return ans;
	}

	public static int f(int[] arr,int l,int r,boolean is_l){
		int v=is_l?arr[l]:arr[r];
		while(l-1>=0&&arr[l-1]==v)l--;
		while(r+1<arr.length&&arr[r+1]==v)r++;
		if (l==0&&r==arr.length-1) return 0;
		int p1=arr.length,p2=arr.length;
		if (l-1>=0) p1=f(arr,l-1,r,true);
		if (r+1<arr.length) p2=f(arr,l,r+1,false);
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10000; i++) {
			...
			int ans1 = allSame2(arr);
			int ans2 = allSame3(arr);
			if (ans1 != ans2) {
