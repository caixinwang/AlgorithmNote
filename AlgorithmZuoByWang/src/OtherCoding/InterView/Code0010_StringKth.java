...
public class Code0010_StringKth {
	// 思路：
	...
	public static int kth(String s, int len) {
		...
		for (int i = 0, rest = len - 1; i < num.length; i++, rest--) {
			ans += (num[i] - 'a') * f(rest) + 1;//相等于往右走加上往下走
		}
		return ans;
	}
	...
	public static int kth2(String s, int len) {
		if (s == null || s.length() == 0 || s.length() > len) {
			return -1;
		}
	...
	// 长度为len,1+26+...+26^(len-1)=(26^len-1)/25
	public static int g(int len) {
		return (int)((Math.pow(26,len)-1)/25);
	}
