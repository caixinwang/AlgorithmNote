...
public class Code0015_ReverseInvertString {
	...
	public static char kth(int n, int k) {
		...
		int half = lens[n - 1];// Sn= Sn-1  L[n]  Sn-1
		if (k <= half) {
			return kth(n - 1, k);
		} else if (k == half + 1) {
			return (char) ('a' + n - 1);//等差数列公式，首相'a'+d,d=n-1
		} else {//找右边部分的第k-(half+1)个，在reverse之前在前半部分的 (half+1)-(k-(half+1))=2*(half+1)-k 个
			return invert(kth(n - 1, ((half + 1) << 1) - k));
		}
	}

	public static char invert(char c) {// 'a' -> 'y'
		return (char)('a'+'y'-c);//中心对称
	}
