...
public class Code0024_ArrangeProject {
	...
	public static int dayCount2(ArrayList<Integer>[] nums, int[] days, int[] headCount) {
		Queue<Integer> queue=new LinkedList<>();
		for (int i=0;i< headCount.length;i++){
			if (headCount[i]==0) queue.add(i);
		}
		int ans=0;
		while (!queue.isEmpty()){
			int size= queue.size();
			...
			for (int i = 0; i < size; i++) {
				...
				max = Math.max(max, days[poll]);//决出这一层的最大值，就是瓶颈
				for (Integer next: nums[poll]){
					if (--headCount[next]==0) queue.add(next);
				}
			}
			ans+=max;
		}
