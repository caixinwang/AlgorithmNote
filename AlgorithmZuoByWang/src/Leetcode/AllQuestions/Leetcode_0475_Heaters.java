...
public class Leetcode_0475_Heaters {
	//技巧：不回退。排完序之后，如果x供暖器没有比x+1给i供暖好，那么x更不可能给i后面的供暖。所以x可以直接杀死。满足了单调性
	//如果x位置供暖效果比x-1的供暖效果差，那么说明i最好就是x-1位置去供暖。x位置应该给i+1位置供暖了。
	//这题如果供暖效果一样，一定要跳下去。因为如果不跳，当火炉重叠的时候会出现错误！会被锁死在原地。
