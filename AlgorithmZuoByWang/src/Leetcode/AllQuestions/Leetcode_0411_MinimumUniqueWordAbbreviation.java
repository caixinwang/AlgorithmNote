...
public class Leetcode_0411_MinimumUniqueWordAbbreviation {
	...
	public static String minAbbreviation1(String target, String[] dictionary) {
		...
		int index = 0;
		for (String word : dictionary) {
			if (word.length() == len) {//只有长度和我一样的才会和我撞！
				char[] w = word.toCharArray();
				int status = 0;
				for (int j = 0; j < len; j++) {
					if (t[j] != w[j]) {//不一样的位设置为1。最终答案保留的位至少有一个与他们不一样，也就是与完之后不为0。
						status |= 1 << j;
					}
				}
				words[index++] = status;
