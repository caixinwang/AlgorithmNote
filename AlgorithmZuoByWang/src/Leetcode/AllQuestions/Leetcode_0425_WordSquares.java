...
public class Leetcode_0425_WordSquares {
	//方法：所有单词的所有前缀都搞成一张表。不要忘记了，前缀为空的时候所有单词都有，这是边界条件
	//敢从0位置开始掉，一定要考虑空串的时候
	...
	public static List<List<String>> wordSquares2(String[] words) {
		List<List<String>> ans=new ArrayList<>();
		HashMap<String,List<String>> map=new HashMap<>();
		for(String s:words){//所有前缀
			for(int len=0;len<=s.length();len++){
				String substr=s.substring(0,len);
				if(!map.containsKey(substr)) map.put(substr,new ArrayList<>());
				map.get(substr).add(s);
			}
		}
		int n=words[0].length();
		dfs(n,0,new LinkedList<>(),ans,map);
		return ans;
	}

	public static void dfs(int n,int index,
					LinkedList<String> path,List<List<String>> ans,HashMap<String,List<String>> map){
		if(index==n){
			ans.add(new ArrayList<>(path));//path用LinkedList，ans用ArrayList会更省空间
			return;
		}
		...
		if (map.containsKey(substr)){
			for(String s:map.get(substr)){
				path.addLast(s);
				dfs(n,index+1,path,ans,map);
				path.pollLast();
			}
		}
	}
