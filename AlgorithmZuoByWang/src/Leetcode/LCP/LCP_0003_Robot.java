...
public class LCP_0003_Robot {
	//看到题目的数据量就知道，不能模拟！所以我们就得搞出一个函数f，可以迅速的搞定机器人能不能到达x,y这个点。
	//关键点：周期！！算出一个周期内x的移动量n以及y的移动量m，以及把一轮以内x和y移动的组合存在set里面。
	//这样我们就可以把任意给定的(x,y)通过n与m划分到我们的一轮内，然后在表里查有没有这种可能性。
	//如何把x和y的组合存在set里面呢，最简单的做法肯定是存字符串"x_y"，但是由于command<=1000，所以x和y最大1000
	//所以我们可以把x和y揉在一个int里面，x<<10|y来代替x_y，整数肯定比String快多了！！！
	//还可以继续优化，我们连set都不需要。x<<10|y最多20位，2^20-1=1048575。我们可以用一个bit，来表示某一个数出现与否
	//需要1048575个bit。一个int32位，那么我们需要准备1048575/32  bits，也就是一个1048575/32大小的整型数组。
