...
先看典型的应用场景再看如何推广。给定一个有1~1000下标的数组，做出一个线段树结构，对外提供三个接口。
...
前面元素个数是2的次幂，现在希望在元素个数不是2的次幂的时候，都希望构建出来的这棵树是满二叉树。举个例子，假设现在只有6个数，我可以认为最底层还是8个数，但是我认为第7个数和第8个数都是0，但是对超出的范围不进行任何操作。这么做是因为只有元素个数刚好是2的次幂的时候才能够每次二分都能够均分。

对于任意元素的个数N，预留4N的空间用来构造树肯定够用，那么我每次底层都准备4N的空间来装树的结点，推导如下

```
2^m < N < 2^(m+1)
总结点数=2*(2^m)+1
4N > 4*(2^m) = 2*(2^m)+2*(2^m) >= 2*(2^m)+2 >  2*(2^m)+1 = 总结点数
```

<img src="image/image-20240808235634946.png" alt="image-20240808235634946" style="zoom:50%;" />

满二叉树用数组表示，因为满二叉树有一个优雅的性质，结点i的父节点为i/2，左孩子为2i，右孩子为2i+1。
