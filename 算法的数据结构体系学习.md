...
```
自己作为右边界，最右边的1和最右边的数互换之后得到左边界。奇数运用这个规则得到自己，偶数得到一个范围。
1，二进制是0001，奇数，管自己
...
偶数的右边界是自己。左边界的规律为找到最右边的1，然后和最后一位（一定是0）的位置互换一下，例如12->1100，互换完得到左边界1001
101110001100000右边界互换玩得到左边界101110001000001         
```
...
1001~1100
0001~1000
你会发现左边界可以通过计算得出，所以可以省略左边界，我们只关注右边界，进而直接将右边界映射到indexTree数组中，对应上述的拆分规则。
