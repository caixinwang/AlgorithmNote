...
也可以使用一个有踢人行为的单调队列来找出所有的候选，遍历的方向与单调栈的相反。例如，固定左端点，要找出所有候选的右端点，单调栈是从右往左遍历累加和数组。而单调队列是从左往右遍历累加和数组。单调栈的逻辑是，最右边的端点是最长的，往左走肯定长度不如它，但是要更容易满足条件。单调队列的逻辑是，往右边走，长度已经变长了，如果还比左边的更容易满足条件，那么就可以淘汰左边的。
...
枚举方式：对于每一个位置枚举它作为左端点和右端点的答案。

对于一个位置 i ，如果i作为右端点，那么 从i-1 ~ 0 这个方向需要维持一个递增，可以用单调队列，i从0开始遍历，可以动态维护好这个递增的关系。一旦i与队头产生一个答案，队头需要退出。i在入队前作为右端点考虑，入队后按照左端点考虑，由于入队之后按照左端点考虑所以需要从队尾踢出比等于等于它的。

最短的子数组问题需要考虑一个位置i作为左端点和右端点的答案。而最长的问题，把所有可能作为右端点或者左端点的位置都放到单调栈里面了，所以只需要考虑一个位置i作为某一端的答案。
...
滑动窗口的解法本质上就是枚举固定子数组左端点或者右端点的答案。寻找单调性可以证明不回退。

如果固定左端点，那么可以从左往右扩到不能再扩。如果窗口的含义是[ l , r )那么也就是r位置的字符已经在前面出现过了。此时 l 位置开头的答案就产生了，就是r-l。r以及后面的位置都不可能，因为必定重复。r也不需要回退，因为l~r-1位置是没有重复的，故新的左端点l+1~r-1位置也是没有重复的。
...
我们指定正好包含types种字符，并且每种字符至少重复k次，这样我们就可以维持一个种类diff，以及all进行计数。

举例的窗口：[L,R)，也就是R位置是待加入的元素

条件：子串中每种字符都至少出现k次
条件分析：子串与扩张没有单调的联系，扩张可能导致不满足条件也可能导致满足条件。
我们需要固定字符的种类才能维持单调性。如果不固定字符种类，固定左端点L的情况下，不知道R到哪里才停，换句话说，如果R位置不满足条件，不能排除R右边的答案。例如，R 位置是一个新的字符，加入导致子串不达标，但是你必须继续往右扩，因为往右之后可能就达标了。

条件：子串中必须恰好types种字符，并且每种字符至少出现k次
条件分析：子串必须一直扩张，先满足种类，在满足种类的情况下尽可能扩展去满足个数。条件与扩张具有单调的联系，随着扩张，先从不满足条件到满足条件（个数<=k 或 种类<=types），再从满足条件到不满足条件（种类>types）。
必须要满足types种字符，固定左端点L的情况下，一直扩张到即将不满足条件，也就是R位置如果加入就不满足条件了（种类>types）。由于单调性，R后面的位置也不可能满足条件。并且右端点可以不回退，因为 1. 如果之前的 L-1作为左端点的子串达标，那么当前 L 作为左端点子串只能比它更长才行，也就是它必须要能扩张才可能作为答案。2. 如果之前 L-1 作为左端点的子串不达标，那么不是少数目就是少种类，l作为左端点并且没有扩张之前只可能比之前更差，所以不回退。
产生答案的时机在退出循环或者R扩张的时候，需要检查当前状态的窗口是否满足条件。
...
固定左端点L的情况下，R不断往右边扩张，直到字符种类即将大于k才停，也就是R位置是一个新字符。具有单调性，R右边的一定也不是L的答案。并且L+1为左端点的时候R不回退，因为此前L已经与R产生了一个答案，一定会比中间的好，L开头如果不能扩张就一定不是答案。

产生答案的时机在于扩张的时候，不需要检查是否满足条件（因为扩张的途中的窗口一定满足条件）
