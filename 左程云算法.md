...
所以卡特兰数的问题，大部分它就变成了公式二的找敏感度的问题。其中最重要的敏感度就是任何一个前缀上，零的个数都不要少于一的个数。这个经典问题无数无穷无尽的在每年的面试过程中，以一个新的样子出现，难倒了一大批观众。
...
子数组总数N²，并且你不能再题目给的arr[l...r]去原地排序，因为你可能破坏原本arr的结构。所以你如果要排序，只能复制出一个子数组，然后排序一下，排序验证是一个n log n 的复杂度，验证是一个N的复杂度。所以总的就是						N² * N * log N的复杂度，暴力方法复杂度完全不能接受。那么你能不能把这个可整合数组的定义等价变换一下？
...
![image-20230331174945269](image/image-20230331174945269.png)
