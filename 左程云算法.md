...
正常来说，我们就是直接l++,r--即可。但是这题，我们移动到的下一个位置不能和前面的位置相等，所以我们用一个空体的while来解决这个问题，它会一直移动到越界或者一个合适的位置。

注意：使用--i和++i。

条件为`++l<r&&nums[l]==nums[l-1]`。第一段为`++l<r`,这和外循环的条件相符，意思是你移动到的位置不能超出外循环的条件。并且你要移动到`nums[l]==nums[l-1]`一个这样合适的位置才停。
